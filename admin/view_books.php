            ...
            if(mysqli_num_rows($query_run) > 0)
            {
                ...
                foreach($query_run as $user)
                {
                    ...
                    <tr>
                        ...
                        <td><?= $user['author']; ?></td>
                        <td>
                            <a href="book_edit.php?id=<?= $user['id']; ?>" class="btn btn-success btn-sm">Edit</a>
                            <form action="../action.php" method="POST" class="d-inline">
                                <button onclick="return confirm('Are you sure you wanna delete book?')" style="width: 60px" type="submit" name="delete_book" value="<?=$user['id'];?>" class="btn btn-danger btn-sm">Delete</button>
                            </form>
                        </td>
                    </tr>
                    <?php
